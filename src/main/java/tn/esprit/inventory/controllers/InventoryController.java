...
import tn.esprit.inventory.services.AIService;
import tn.esprit.inventory.services.InventoryService;
import tn.esprit.inventory.services.PDFService;
import tn.esprit.inventory.services.QRCodeService;
import tn.esprit.inventory.services.WeatherService;        // ← NEW
import tn.esprit.navigation.Router;
import tn.esprit.navigation.Routes;
import tn.esprit.utils.DialogUtils;
...
public class InventoryController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        // Apply role-based visibility (ADMIN vs USER)
    ...
    private void applyRoleBasedVisibility() {
        ...
        // USER: hide admin-only top bar buttons
        if (addButton != null) {
        ...
        if (weatherButton != null) {
            ...
            weatherButton.setManaged(false);
        }
    ...
    private void goToRentals() {
        // Preferred path when running inside the shared MainLayoutController / Router
        try {
            Router.go(Routes.rentals);
            return;
        } catch (Exception ignored) {
            // If Router is not initialized (e.g. running Inventory standalone),
            // we fall back to replacing the current Scene root directly.
        }
        ...
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/fxml/inventory/RentalView.fxml"));
            Parent rentalRoot = loader.load();

            scene.setRoot(rentalRoot);

            try {
                String rentalCss = getClass()
                        .getResource("/styles/rental-styles.css")
                        .toExternalForm();
                if (!scene.getStylesheets().contains(rentalCss)) {
                    scene.getStylesheets().add(rentalCss);
                }
            } catch (Exception ignored) {
                // If the stylesheet is missing we just skip it;
                // the Rentals view will still function.
            }

            if (scene.getWindow() instanceof Stage stage) {
                stage.setTitle("AgriRent - Rental Management");
            }
        } catch (Exception e) {
            e.printStackTrace();
            showErrorAlert("Failed to open Rentals view.");
        }
    }
